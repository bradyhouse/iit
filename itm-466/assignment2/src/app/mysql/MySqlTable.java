...
/**
 * Class used to virtualize a MySql Server Table.
 * 
 */
public class MySqlTable extends MySqlEntityBaseClass {
		...
		try {
			...
			if (this.getXsdFile() != null) {
				...
				if (this.getXmlFile() != null) {
					Document _xmldoc = docBuilder.parse(new File(this
							.getXmlFile()));
	...
	/**
	 * Private getter used to generate an SQL Create Table statement from the
	 * table's name and columns collection.
	 */
	private String getSqlCreateStatement() {
		String _sqlStatement = "CREATE TABLE `"
				+ this.getMySqlConnection().getDatabase() + "`.`"
				+ this.getName() + "` (\n";
		Integer _lastColumn = this.getDataColumns().size() - 1, _currentColumn = 0;

		for (MySqlColumn _col : this.getDataColumns().values()) {
			_sqlStatement += _col.toSql()
					+ (_currentColumn.equals(_lastColumn) ? "\n" : ",\n");
			_currentColumn++;
		}
		_sqlStatement += ");";
	...
	/**
	 * Private method used to create the table on the database. It returns true
	 * if (a) the connection object exists, (b) the isXmlValid method returns
	 * true and (c) the create statement sql executes successfully. Otherwise,
	 * it returns false.
	 * 
	 ...
	 * @throws ParserConfigurationException
	 */
	private boolean createDatabaseTable() throws SQLException,
	...
	/**
	 * Control method used to initiate the table creation process on the target
	 * database.
	 */
	...
	/**
	 * Override of the class to string in order to return the Table's SQL Create
	 * Statement.
	 */
	@Override
