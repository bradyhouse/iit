 ...
 * config.xml file for MySql Connection details.
 * 
 */
public class XmlMySqlConnectionParseHandler extends
		XmlConfigParseHandlerBaseClass {
	...
	public void characters(char ch[], int start, int length)
		...
		if (this.isMyElement()) {
			...
			case PORT: {
				this.m_cls_connection.setPortNumber(new String(ch, start,
						length));
				break;
			}
			case ENTITYNAME: {
			...
			case DATABASE: {
				this.m_cls_connection
						.setDatabase(new String(ch, start, length));
				break;
			}
			case USERNAME: {
				this.m_cls_connection
